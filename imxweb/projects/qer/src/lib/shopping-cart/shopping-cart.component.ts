...
import { CartCheckResult, CheckMode, ITShopConfig, PortalCartitem, PortalItshopCart } from '@imx-modules/imx-api-qer';
import { ClassloggerService, ConfirmationService, LdsReplacePipe, SnackBarService, calculateSidesheetWidth } from 'qbm';
...
export class ShoppingCartComponent implements OnInit {
  ...
  public async submitShoppingCart(): Promise<void> {
    ...
    if (validator.hasWarnings) {
      ...
      const submitRef = this.dialogService.open(ShoppingCartSubmitWarningsDialog, {
        data: {
    ...
    if (!confirmSubmit) {
      this.snackBarService.open({ key: '#LDS#You have canceled the submission of your shopping cart.' }, '#LDS#Close');
      await this.getCartItems();
      return;
    }
    ...
    try {
      ...
      this.snackBarService.open({ key: '#LDS#Your shopping cart has been successfully submitted.' }, '#LDS#Close');
  ...
  public async getData(reloadAll: boolean): Promise<void> {
    ...
    try {
      if (reloadAll) {
        ...
        const preSelectedCart =
          this.selectedItshopCart == null
            ? null
            : this.shoppingCartCandidates.find((cart) => cart.DocumentNumber.value === this.selectedItshopCart.DocumentNumber.value);
  ...
  private async getCartItems(): Promise<void> {
    if (this.selectedItshopCart) {
      ...
      this.logger.debug(this, 'getCartItems - shopping cart loaded', this.selectedItshopCart.GetEntity().GetDisplay());
    } else {
  ...
  private async checkTermsOfUse(): Promise<boolean> {
    ...
    const itemsWithTermsOfUseToAccecpt = this.shoppingCart.getItems(
      (item) =>
    ...
    if (!!itemsWithTermsOfUseToAccecpt?.length) {
      this.logger.debug(
        this,
