...
import { CartItemCheckResult, CartItemDataRead, PortalCartitem } from '@imx-modules/imx-api-qer';
...
export class ShoppingCart {
  ...
  public getItems(filter: (item: PortalCartitem) => boolean): PortalCartitem[] | undefined {
    return this.itemCollection && this.itemCollection.Data ? this.itemCollection.Data.filter(filter) : undefined;
  }

  public getItemsSorted(): PortalCartitem[] | undefined {
    if (this.itemCollection && this.itemCollection.Data) {
      return this.itemCollection.Data.map((cartItem) => ({
  ...
  public getCartItemCheckResult(cartItem: PortalCartitem): CartItemCheckResult | undefined {
    return this.itemCollection && this.itemCollection.extendedData && this.itemCollection.extendedData.CheckResults
      ? this.itemCollection.extendedData.CheckResults.find(
          (item) => item != null && item.UidShoppingCartItem === cartItem.GetEntity().GetKeys()[0],
        )
      : undefined;
  }
  ...
  public getItem(uid: string): PortalCartitem | undefined {
    return (
      this.itemCollection && this.itemCollection.Data && this.itemCollection.Data.find((item) => item.GetEntity().GetKeys()[0] === uid)
    );
  }
