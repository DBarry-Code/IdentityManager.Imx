...
describe('ApproverContainer', () => {
  ...
  for (const testcase of [
    ...
    {
      descriprion: 'with empty values',
      pwo: buildPwo('uidQerWorkingMethode', 0, [], [], []),
      personNow: [],
      personsFuture: [],
    },
    {
      descriprion: 'with single step',
      pwo: buildPwo(
        'uidQerWorkingMethode',
        0,
        [buildStep('uidQerWorkingMethode', 0, 0, 'uidStep')],
    ...
    {
      descriprion: 'with multiple steps (only 1 match)',
      pwo: buildPwo(
        'uidQerWorkingMethode',
        0,
    ...
    {
      descriprion: 'with multiple steps (but only 1 approver)',
      pwo: buildPwo(
        'uidQerWorkingMethode',
        0,
        [buildStep('uidQerWorkingMethode', 0, 0, 'uidStep'), buildStep('uidQerWorkingMethode', 1, 1, 'uidStep2')],
        [buildData('uidStep', 'person'), buildData('uidStep2', 'person2')],
        ['person'],
      ),
      personNow: ['person'],
      personsFuture: [],
    ...
    {
      descriprion: 'with multiple steps (future and now)',
      pwo: buildPwo(
        'uidQerWorkingMethode',
        0,
      ...
      personNow: ['person'],
      personsFuture: ['person2'],
    },
    {
      descriprion: 'with multiple steps (multiple future and now)',
      pwo: buildPwo(
        'uidQerWorkingMethode',
        0,
        [
      ...
      personNow: ['person', 'person2'],
      personsFuture: ['person2', 'person3'],
    },
  ]) {
  ...
  it('can initialize with multiple approvers (now)', () => {
    ...
    const steps = [buildStep(uidQerWorkingMethod, level, positiveSteps, stepUid)];
  ...
  it('can initialize with multiple future steps', () => {
    ...
    personsFuture.forEach((approver, index) =>
      expect(container.approverFuture[index].Columns.UID_PersonHead.DisplayValue).toEqual(approver),
    );
  });
  ...
  it('can initialize with auto steps', () => {
    ...
    const steps = stepUids.map((stepUid, index) => buildStep(uidQerWorkingMethod, level + index * positiveSteps, positiveSteps, stepUid));
