...
@Injectable()
export class FilterSqlWizardService extends SqlWizardApiService {
  constructor(private readonly api: QerApiService) {
    super();
  }

  public async getFilterProperties(table: string): Promise<FilterProperty[]> {
    return (await this.api.client.portal_sqlwizard_tables_columns_get(table)).Properties || [];
  }

  public async getCandidates(parentTable: string, options?: CollectionLoadParameters): Promise<EntityCollectionData> {
    return await this.api.v2Client.portal_sqlwizard_candidates_get(parentTable, options);
  }
}
