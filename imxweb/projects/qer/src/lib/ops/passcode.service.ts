...
import { PersonPasscodeResult } from '@imx-modules/imx-api-qer';
...
export class PasscodeService {
  ...
  public async showPasscode(passcode: PersonPasscodeResult, userDisplay: string, managerDisplay: string): Promise<void> {
    return this.openDialogAndShowPasscode(
      passcode.PasscodeSplit,
      passcode.PassCode ?? '',
      managerDisplay,
      userDisplay,
      passcode.HoursValid,
  ...
  private async openDialogAndShowPasscode(
    ...
    const value = this.ldsReplace.transform(await this.translationProvider.get('#LDS#Passcode for {0}').toPromise(), display);
