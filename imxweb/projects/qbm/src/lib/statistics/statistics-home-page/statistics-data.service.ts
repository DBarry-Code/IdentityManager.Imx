...
import { ChartAreaData, ChartDto, HeatmapSummaryDto } from '@imx-modules/imx-api-qbm';
...
export class StatisticsDataService {
  ...
  public searchFavStats$: BehaviorSubject<GenericStatisticEntity[]> = new BehaviorSubject<GenericStatisticEntity[]>([]);
  public favStats$: BehaviorSubject<GenericStatisticEntity[]> = new BehaviorSubject<GenericStatisticEntity[]>([]);
  ...
  constructor(
    ...
    this.isFavoritesEnabled = this.statisticsApi.isFavoritesEnabled;
    this.isOrgStatsEnabled = this.statisticsApi.isOrgStatsEnabled;
  }

  /**
   *
   * @returns The help context id for the statistics page for the respective portal
   */
  public getHelpContextId(): HelpContextualValues {
    return this.statisticsApi.getHelpContextId();
  }

  public getNodeByLeafId(leafId: string): GenericStatisticNode {
    let namedNode = this._flatTree.find((node) => node.leafId === leafId);
    if (!namedNode) {
      this.logger.error(this, 'Could not find the leafId: ' + leafId);
      namedNode = this.getNodeByLeafId(this.groupedAreaId);
    }
    return namedNode;
  }
  ...
  public addToTree(): void {
    ...
    this.dataSource.Data.forEach((stat: GenericStatisticEntity) => {
      ...
      if (area && area.length > 0 && thisTree) {
