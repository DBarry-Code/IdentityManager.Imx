...
import { CollectionLoadParameters, EntityData, IEntityColumn } from '@imx-modules/imx-qbm-dbts';
...
export class MultiSelectFormcontrolComponent implements ControlValueAccessor, OnChanges, OnDestroy, AfterViewInit {
    ...
    this.subscriptions.push(
      this.viewport.renderedRangeStream.subscribe(async (range: ListRange) => {
        if (range.end === this.settingsService.DefaultPageSize + (this.parameters.StartIndex ?? 0)) {
          if (!this.parameters.StartIndex) this.parameters.StartIndex = 0;
  ...
  private async putNewValue(): Promise<void> {
    const value =
      this.selectedCandidates && this.selectedCandidates.length > 0
