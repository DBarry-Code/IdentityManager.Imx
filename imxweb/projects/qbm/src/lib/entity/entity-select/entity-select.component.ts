...
export class EntitySelectComponent implements OnChanges, AfterContentInit {
      ...
      if (this.entities) {
        this.options = this.entities.map((entity) => ({
          display: this.display?.primary
            ? entity.GetColumn(this.display.primary.ColumnName ?? '').GetDisplayValue() || entity.GetDisplay()
            : entity.GetDisplay(),
          displayDetail: this.display?.secondary ? entity.GetColumn(this.display.secondary.ColumnName ?? '').GetDisplayValue() : undefined,
          value: entity,
        }));
      } else {
  ...
  public filter(option: EuiSelectOption, searchInputValue: string): boolean {
    return (
      (option.display.toString().toUpperCase().trim().includes(searchInputValue.toUpperCase().trim()) ||
        option.displayDetail?.toString().toUpperCase().trim().includes(searchInputValue.toUpperCase().trim())) ??
      false
    );
  }

  public selectionUpdated(arg: EuiSelectOption | EuiSelectOption[]) {
    if (Object.hasOwn(arg, 'value')) {
      // is a EuiSelectOption
      this.selectionChange.emit((arg as EuiSelectOption).value);
    } else {
      this.selectionChange.emit(arg[0].value);
    }
  }
}
