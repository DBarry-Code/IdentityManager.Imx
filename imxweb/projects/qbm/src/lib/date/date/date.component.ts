...
export class DateComponent implements OnInit, OnDestroy {
    ...
    this.subscriptions.push(
      this.control.valueChanges.subscribe((x) => {
        if (this.shadowText.untouched) {
          this.shadowText.markAsTouched({ emitEvent: false });
        }
        this.isUpdated = true;
        this.handleControlChanged();
      }),
  ...
  private handleControlChanged(): void {
    ...
    if (!updatedValue?.isSame(this.result)) {
   ...
   */
  private validateMomentInDateRange(date: Moment | undefined): ValidationErrors | null {
    ...
    if (this.min && date && date < moment(this.min)) {
      return { matDatepickerMin: true };
    }

    if (this.max && date && date > moment(this.max)) {
