...
export class ServiceCategoryTreeDatabase extends TreeDatabase {
  ...
  private async loadRoot(showLoading: boolean) {
    ...
    try {
      ...
      const serviceCategories: TypedEntityCollectionData<PortalServicecategories> = await this.serviceCategoriesProvider.get(opts);
      entities = {
  ...
  private async loadData(showLoading: boolean, parameters: CollectionLoadParameters): Promise<TreeNodeResultParameter> {
    let entities: TreeNodeResultParameter;
