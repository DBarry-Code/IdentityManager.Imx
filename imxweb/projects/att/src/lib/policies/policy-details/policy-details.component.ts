...
export class PolicyDetailsComponent {
  ...
  constructor(@Inject(EUI_SIDESHEET_DATA) public readonly data: { policy: PortalAttestationPolicy }) {
    this.uidAttestationPolicy = data.policy.GetEntity().GetKeys()[0];
    this.status = data.policy.IsProcessing.value
      ? '#LDS#Processing'
      : data.policy.CountCases.value === 0
      ? '#LDS#Not run yet'
      : data.policy.CountOpenCases.value === 0
      ? '#LDS#Completed'
      : '#LDS#Pending';
  }
}
