...
import { CompareOperator, ExtendedTypedEntityCollection, FilterType, IEntity, TypedEntity } from '@imx-modules/imx-qbm-dbts';
...
export class TargetSystemDynamicMethodService {
  ...
  public async getById<TEntity extends TypedEntity, TExtendedData = any>(
    ...
    return (
      await this.dynamicMethod.getInteractive(
        this.tsbClient.apiClient,
        { type, key, path },
        {
  ...
  public async get<TEntity extends TypedEntity, TExtendedData = any>(
    ...
    return (await this.getCollection<TEntity, TExtendedData>(type, dbObjectKeyWrapper.dbObjectKey.TableName, { filter }))?.Data[0];
  }
