...
import { ContributingEntitlement, UiActionData } from '@imx-modules/imx-api-cpl';
...
export class ResolveComponent implements OnInit {
  ...
  public async ngOnInit(): Promise<void> {
    ...
    try {
      this.entitlements = await this.cplApi.client.portal_rules_violations_entitlements_get(
        this.data.uidPerson,
        this.data.uidNonCompliance,
      );
  ...
  private async LoadEntitlementsLoseAlso(): Promise<void> {
    ...
    this.busy = true;
    try {
      this.entitlementsLoseAlso = await this.cplApi.client.portal_rules_violations_analyzeloss_post(
        this.data.uidPerson,
        this.data.uidNonCompliance,
        {
          ObjectKeys: this.selectedEntitlements,
  ...
  public LdsLoseEntitlements =
    '#LDS#The identity will lose the following entitlements when the selected actions take effect. Check the list to avoid unintentional loss of access. To change the selection, go back to the previous page.';

  public LdsNoPermissions =
    '#LDS#No entitlements were found. The cause for the violation may has already been resolved. You may close this wizard.';

  public LdsContributingPermissions =
    '#LDS#The following entitlements contribute to this rule violation. Select the entitlements to be removed from the identity.';
